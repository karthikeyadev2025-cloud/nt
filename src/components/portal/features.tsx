        ...
        <div className="space-y-2">
          {mine.map(m => (
            <div key={m.id} className={cardCls + ' flex items-center justify-between'}>
              <p className="text-stone-700 text-sm">{m.shift_date} {m.target_id && `↔ ${byId[m.target_id] || '—'}`}</p>
              <span className={`text-xs ${statusColor[m.status]}`}>{m.status}</span>
            </div>
          ))}
          {mine.length === 0 && <p className="text-stone-700 text-sm">No requests yet.</p>}
        </div>
      </div>
    </div>
  );
      ...
      {!editing ? (
        ...
        <div className="space-y-2">
          ...
          <div className="flex gap-2">
            ...
            <button className="text-stone-700 text-sm" onClick={() => setEditing(false)}>Cancel</button>
          </div>
        </div>
      )}
    </div>
...
export function BankChangeApprovals() {
  ...
  return (
    <div className="space-y-2">
      ...
      {items.length === 0 && <p className="text-stone-700 text-sm text-center py-10">No bank change requests.</p>}
    </div>
  );
}
      ...
      {tab === 'jobs' && (
        <div>
          ...
          <div className="space-y-2">
            ...
            {jobs.length === 0 && <p className="text-stone-700 text-sm text-center py-10">No job postings yet.</p>}
          </div>
        </div>
      )}
...
export function PhotoChangeApprovals() {
  ...
  if (pending.length === 0) return <p className="text-stone-700 text-sm text-center py-10">No pending photo change requests.</p>;
