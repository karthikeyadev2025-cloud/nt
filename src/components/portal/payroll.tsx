...
export function ShiftsManager({ segments }: { segments: { slug: string; name: string }[] }) {
  ...
  const [assignments, setAssignments] = useState<Tables<'staff_shifts'>[]>([]);
  const [editing, setEditing] = useState<Partial<Tables<'shifts'>> | null>(null);
  ...
  async function save() {
    ...
    if (editing.id) {
      ...
      ({ error } = await supabase.from('shifts').update(patch as never).eq('id', id!));
    } else {
      ({ error } = await supabase.from('shifts').insert(editing as never));
    }
...
export function PayslipManager() {
  ...
  async function autoFillFromAttendance() {
    ...
    // Fire the per-leave RPCs in parallel — sequential await inside a for
    // loop turned a 5-leave month into 5 serial round-trips over what's
    // typically a slow India-mobile connection.
    const leaveResults = await Promise.all(
      (leaves || []).map(async l => {
        ...
        return { leave_type: l.leave_type, days: Number(lwd ?? 0) };
      })
    );
    let paidLeave = 0, unpaidLeave = 0;
    for (const r of leaveResults) {
      if (r.leave_type === 'unpaid') unpaidLeave += r.days; else paidLeave += r.days;
    }
    const absent = Math.max(0, workingDaysInMonth - present - paidLeave - unpaidLeave);

    // Late fine from the staff's currently-assigned shift. Before this,
    // shift.late_fine_amount was configurable in ShiftsManager but the
    // generator only used the manually-entered genForm.late_fine (default
    // 0), so late-fine deductions never actually landed on autofilled
    // payslips. Two supported fine types: 'per_day' (× late_days) and
    // 'fixed' (flat amount if any late days). 'none' → 0.
    const { data: shiftRow } = await supabase
      .from('staff_shifts')
      .select('shifts(late_fine_type, late_fine_amount)')
      .eq('staff_user_id', genForm.staff_user_id)
      .is('effective_to', null)
      .maybeSingle();
    // Narrow the joined-row shape at the boundary. supabase-js widens
    // relational selects to Json until types are regenerated.
    type ShiftFineJoin = { shifts: { late_fine_type: string | null; late_fine_amount: number | null } | null };
    const shift = (shiftRow as ShiftFineJoin | null)?.shifts;
    let lateFine = genForm.late_fine;
    if (shift && lateDays > 0) {
      if (shift.late_fine_type === 'per_day') {
        lateFine = (shift.late_fine_amount || 0) * lateDays;
      } else if (shift.late_fine_type === 'fixed') {
        lateFine = shift.late_fine_amount || 0;
      }
    }

    setGenForm({
      ...genForm, working_days: workingDaysInMonth, present_days: present,
      absent_days: absent, paid_leave_days: paidLeave, unpaid_leave_days: unpaidLeave,
      late_days: lateDays, late_fine: lateFine,
    });
...
export function AttendanceDetailsModal({ staffUserId, staffName, onClose }: { staffUserId: string; staffName: string; onClose: () => void }) {
  const [logs, setLogs] = useState<Tables<'attendance_records'>[]>([]);
...
export function AttendanceSummaryTable({ segments }: { segments: { slug: string; name: string }[] }) {
  type AttendanceSummaryRow = {
    staff_user_id: string; full_name: string; role: string;
    days_present: number; days_absent: number; days_on_leave: number; attendance_pct: number;
  };
  ...
  useEffect(() => {
    cachedRpc(`staff_attendance_summary:${segment}:${days}`, () =>
      ...
      // The cached wrapper returns the PostgrestResponse verbatim on cache-miss
      // and the .data payload on hit — normalize.
      const data = Array.isArray(res) ? res : (res as { data?: AttendanceSummaryRow[] } | null)?.data;
