/*
  # Surface overdue callbacks/appointments + duplicate leads on the dashboard (2026-08-07)

  get_dashboard_counts already computed overdueFollowups but the Overview
  page never actually rendered it as a tile — a real count silently going
  unused. Adding two more in the same spirit: overdue callbacks/
  appointments (the two reminder types added in 20260807000001 that had
  no visibility on this dashboard at all) and how many duplicate-lead
  groups exist (20260807000002's merge tool had no indicator anywhere
  that there was anything to merge — you'd only find out by opening the
  tab and checking).
*/
